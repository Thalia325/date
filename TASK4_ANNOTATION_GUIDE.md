...
本指南用于中文化学试卷、中文化学实验讲义和实验报告模板的人工标注。公开数据中的 M6Doc、ScienceQA、TQA 可用于预训练或辅助理解，但中文实验讲义细粒度结构需要自建标注。

## 标注目标

一页文档同时保留两层信息：

1. 页面结构：这个区域在页面上是什么，例如题目、正文、选项、图、表、公式、答案区。
2. 教育语义：这个内容在化学教学中是什么，例如实验目的、实验步骤、实验现象、实验装置、概念解释。
...
| 标签 | 含义 | 标注口径 |
|---|---|---|
| `page_title` | 页面或文档主标题 | 例如“高一化学期末试卷”“酸碱中和滴定实验”。 |
| `section_title` | 小节标题 | 例如“一、选择题”“实验步骤”。 |
| `paragraph` | 普通正文 | 不属于题目、选项、图表、实验专门字段的正文。 |
| `figure` | 图片或示意图 | 包括实验装置图、曲线图、结构图、流程图。 |
| `figure_caption` | 图题/图注 | 与最近的 figure 关联。 |
| `table` | 表格 | 包括实验记录表、数据表、选项表。 |
| `table_caption` | 表题/表注 | 与最近的 table 关联。 |
| `formula` | 数学公式或普通公式 | 例如计算公式、比例关系。 |
...
| 标签 | 含义 | 标注口径 |
|---|---|---|
| `question_block` | 完整题目区域 | 包含题号、题干、选项、图表、答案区时可框整个大题。 |
| `question_stem` | 题干 | 不含选项；题干跨行或跨栏时合并为同一题干区域。 |
...
| 标签 | 含义 | 标注口径 |
|---|---|---|
| `experiment_purpose` | 实验目的 | 例如“掌握酸碱中和滴定原理”。 |
| `experiment_principle` | 实验原理 | 包含反应原理、平衡原理、定量关系。 |
...
| `safety_note` | 注意事项/安全提示 | 包括防护、废液处理、危险试剂提醒。 |

若同一段同时包含“步骤”和“注意事项”，优先按主要功能标注；安全风险句可额外作为 `safety_note` 子区域。

## 关系标注

建议为区域之间补充关系，方便后续内容理解：
...
- 化学无关页面是否误保留。
- 同一试卷或同一模板是否重复进入 train/val/test。
- 题干和选项是否切分过粗或过细。
- 反应式、离子方程式、实验装置图是否漏标。
- 实验步骤和实验现象是否混标。
- 答案区和解析区是否混标。
