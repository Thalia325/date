#!/usr/bin/env python3
"""Write readable per-folder dataset review reports under Date/.

This script audits only the competition-facing dataset folders in Date/ and
overwrites each folder's 清洗审查报告.md. It does not modify source data.
"""
...
TASK3_FIELD_RE = re.compile(
    r"reagent|catalyst|solvent|temperature|temp\b|time\b|yield|condition|"
    r"substrate|product|pressure|conversion|selectivity|equiv|molar|reaction",
    re.I,
)
...
def scan_folder(root: Path) -> FolderStats:
    ...
    for path in iter_data_files(root):
        ...
        try:
            ...
            pass
        stats.ext_counts[suffix or "<no_ext>"] += 1
...
def expected_status(root: Path, paths: list[str]) -> list[tuple[str, bool]]:
    rows = []
    for item in paths:
        if any(ch in item for ch in "*?[]"):
            exists = any(root.glob(item))
        else:
...
def count_json_records(path: Path) -> tuple[int | None, str]:
    ...
    return 1, type(payload).__name__
...
def inspect_zip(path: Path, class_names: list[str]) -> dict[str, Any]:
    result: dict[str, Any] = {
        "zip_name": path.name,
        "valid": False,
        "members": 0,
...
def inspect_peace(root: Path) -> dict[str, Any]:
    ...
    image_root = root / "real_world_test_set"
    result: dict[str, Any] = {
        "labels": 0,
        "images": len(list(image_root.rglob("*.png"))) if image_root.exists() else 0,
        "missing_images": 0,
    ...
    for image_ref, label in labels.items():
        ...
        if len(result["samples"]) < 3:
            result["samples"].append(f"{image_ref}: {label[:100]}")
...
def inspect_chemtable(root: Path) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "image_files_in_data_img": len(list(img_dir.glob("*.png"))) if img_dir.exists() else 0,
    }
    ...
    with metadata.open("r", encoding="utf-8", errors="replace", newline="") as f:
        ...
        for row in reader:
            ...
            if '"tables"' in text:
                table_rows += 1
            if TASK3_FIELD_RE.search(text):
                condition_signal_rows += 1
    result.update(
        {
            "metadata_rows": rows,
            ...
            "rows_with_task3_field_signal": condition_signal_rows,
        }
    )
    return result
...
def inspect_chemtables(root: Path) -> dict[str, Any]:
    ...
    for key, path in files.items():
        ...
        result[key] = {"exists": path.exists(), "records": count, "shape": kind}
    return result
...
def inspect_scienceqa(root: Path) -> dict[str, Any]:
    ...
    for item in values:
        ...
        top_topics[item.get("topic", "")] += 1
    return {
        "problem_count": len(data),
        "chemistry_count": chemistry,
        "with_image_field": with_image,
        "local_image_files": len([p for p in (root / "images").rglob("*") if p.is_file()])
        if (root / "images").exists()
        else 0,
        "splits": dict(split_counts),
        "top_topics": dict(top_topics.most_common(6)),
    }
...
def inspect_scemqa(root: Path) -> dict[str, Any]:
    result: dict[str, Any] = {}
    for key, relative in {
        "chem_multiple_choice_json": "chem/chem_multiple_choice.json",
        "chem_free_response_json": "chem/chem_free_response.json",
        "test_multiple_choice_jsonl": "test_chem_multiple_choice.jsonl",
        "test_free_response_jsonl": "test_chem_free_response.jsonl",
    }.items():
        path = root / relative
...
def detail_for(spec: DatasetSpec, root: Path) -> dict[str, Any]:
    if spec.dataset == "Chemical Images Classifier":
        ...
        return {"zip": inspect_zip(zip_path, ["one_molecule", "several_molecules", "reactions", "other"])} if zip_path.exists() else {}
...
def verdict_for(spec: DatasetSpec, stats: FolderStats, detail: dict[str, Any]) -> tuple[str, str]:
    if stats.data_files == 0:
        return "不符合", "目录中没有发现除审查报告以外的有效数据文件，当前不能用于该任务。"
    if spec.dataset == "M6Doc":
        if stats.image_count == 0 and not any((Path("dummy") / p) for p in []):
            return "部分符合", "本地仅有说明/申请材料，缺少页面图像和区域标注，不能直接训练或严格验证。"
    if spec.dataset == "DECIMER-Segmentation":
        return "不符合", "未发现结构图分割图像、mask 或标注文件。"
    if spec.dataset == "Chemical Images Classifier":
        zip_info = detail.get("zip", {})
        class_counts = zip_info.get("class_counts", {})
        needed = ["one_molecule", "several_molecules", "reactions", "other"]
        if zip_info.get("valid") and all(class_counts.get(key, 0) > 0 for key in needed):
            return "部分符合", "四类化学图片 crop 已可识别，可用于二级分类；但不含整页 bbox，不能单独满足区域检测。"
        return "部分符合", "已发现压缩包或分片文件，但需要确认压缩包完整性和类别目录。"
    if spec.dataset == "PEaCE":
        if detail.get("labels", 0) > 0 and detail.get("missing_images") == 0:
            return "部分符合", "real-world test 子集图像和 LaTeX 标签齐全，可做评测和规范化候选；完整训练集未落地。"
        return "不符合", "缺少标签或图片，无法审查化学表达式 OCR 质量。"
    if spec.dataset == "ChemTable":
        if detail.get("metadata_rows", 0) > 0 and detail.get("image_files_in_data_img", 0) > 0:
            return "符合", "表格图像、metadata、reaction/table 字段和任务三字段信号齐全，是任务三主数据源。"
    if spec.dataset == "ChemTables":
        if detail.get("all", {}).get("records") == 788:
            return "部分符合", "适合做表类型路由和负样本筛选，但不是字段级 KIE ground truth。"
    if spec.dataset == "ScienceQA":
        if detail.get("chemistry_count", 0) > 0 and detail.get("local_image_files", 0) > 0:
            return "符合", "题目、答案、解析、split 和本地图像基本齐全，可筛选化学教育题目。"
    if spec.dataset == "SceMQA":
        if detail.get("test_multiple_choice_jsonl", 0) > 0:
            return "符合", "化学多选/自由回答子集和配图已落地，适合任务四题目理解。"
    if spec.task == "任务五":
        return "不符合", "本地目录没有源图像或 graph/box/文本标注，无法映射到 node-edge-parameter schema。"
    return "部分符合", "发现本地文件，但仍需按任务 schema 做字段级转换和抽样复核。"
...
def build_specs() -> list[DatasetSpec]:
    return [
        ...
        DatasetSpec(
            ...
            "补充化学结构区域检测；需要结构图图像及 mask/bbox/segmentation 标注。",
            "化学结构区域检测辅助源",
        ...
        DatasetSpec(
            ...
            "ChemTable",
            "任务三/ChemTable",
            "从反应条件表、底物筛选表、实验参数表中抽取 reagent、catalyst、solvent、temperature、time、yield 等 KIE 字段。",
            "反应条件表格 KIE 主数据",
        ...
        DatasetSpec(
            ...
            "表类型分类与负样本辅助数据",
            "https://data.mendeley.com/datasets/g7tjh7tbrj",
        ...
        DatasetSpec(
            ...
            "M6Doc",
            "任务四/M6Doc",
            "教育文档页面结构化解析；需要页面图像和题目/答案/图表/公式等区域标注。",
            "教育版面结构辅助源",
            "https://github.com/HCIILAB/M6Doc",
            ["annotations", "train2017", "val2017"],
        ...
        DatasetSpec(
            ...
            "ScienceQA",
            "任务四/ScienceQA",
            "化学/科学教育题目理解；需要题干、选项、答案、解析、图片或 caption 上下文。",
            "结构化科学题目理解主数据",
        ...
        DatasetSpec(
            ...
            "RxnScribe",
            "任务五/RxnScribe",
            "化学反应示意图节点、箭头和条件参数解析；需要图像与 graph/box/文本标注。",
            "反应 scheme graph 训练主候选",
            "https://github.com/thomas0809/RxnScribe",
            [],
            "适合将 reactants/products 映射为节点、reaction arrow 映射为边、conditions 映射为参数；本地未落地则不可用。",
        ),
        DatasetSpec(
            "任务五",
            "ReactionDataExtractor2",
            "任务五/ReactionDataExtractor2",
            "作为反应图解析 baseline；需要工具代码、模型或可运行样例输出。",
        ...
        DatasetSpec(
            ...
            "PID2Graph",
            "任务五/PID2Graph",
            "P&ID/PFD 图的节点、管线/边和连接关系解析；需要图像与图结构标注。",
...
def render_detail(detail: dict[str, Any]) -> list[str]:
    lines: list[str] = []
    for key, value in detail.items():
        if key == "zip":
            counts = ", ".join(f"{k}={v}" for k, v in value.get("class_counts", {}).items()) or "无"
            lines.append(
                f"- 压缩包 `{value.get('zip_name')}`：有效={value.get('valid')}，成员={value.get('members')}，"
                f"图片={value.get('images')}，标注/文本={value.get('annotations')}，类别计数：{counts}"
            )
            if value.get("error"):
                lines.append(f"- 压缩包提示：{value.get('error')}")
...
def render_report(spec: DatasetSpec, root: Path, stats: FolderStats, detail: dict[str, Any], report_time: str) -> str:
    ...
    lines = [
        ...
        f"- 二进制分片：{stats.binary_count}",
    ]
    if stats.ext_counts:
        lines.append("- 主要扩展名：" + ", ".join(f"{k}:{v}" for k, v in stats.ext_counts.most_common(10)))
    if stats.truncated:
        lines.append(f"- 注意：文件扫描超过 {MAX_SCAN_FILES} 个，统计已截断。")
    if spec.expected_paths:
        lines.extend(["", "## 必需路径检查", ""])
        for item, exists in expected_status(root, spec.expected_paths):
            lines.append(f"- `{item}`：{'OK' if exists else '缺失'}")

    lines.extend(["", "## 任务适配性审查", "", spec.fit_basis])
    if spec.keep_rules:
        lines.extend(["", "### 保留口径"])
    ...
    lines.extend(["", "## 是否符合当前任务要求", "", f"{verdict}。{reason}", "", "## 后续处理建议", ""])
    if verdict == "不符合":
        lines.append("- 先补齐原始图像、标注和官方 split；补齐后重新运行本地审查。")
    elif verdict == "部分符合":
        lines.append("- 当前数据可进入受限用途；正式训练/评测前需补齐缺口，并做记录级校验。")
    else:
        lines.append("- 可进入下一步清洗转换；转换时保留原始字段、派生字段和质量标记，便于追溯。")
    lines.extend(
        [
