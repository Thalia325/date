...
当前任务方向：化学文档区域识别与分类，覆盖正文、标题、表格、化学方程式、分子结构图、反应路线图、实验步骤、题目区、答案区、图注、表注等区域。

## 当前内容

- `RegionOCR_dataset_screening.md`：M6Doc、DECIMER-Segmentation、Chemical Images Classifier Dataset、PatCID 的可用性筛选结论。
- `DATASET_PREP.md`：任务一数据下载、目录放置、审计、manifest 和 RegionOCR JSONL 转换流程。
- `TASK1_CLEANING_REVIEW.md`：任务一数据清洗审查结论。
- `TASK2_DATASET_PREP.md`：任务二化学表达式 OCR 到 mhchem/JSON 规范化数据准备。
- `TASK2_NEXT_STEPS.md`：任务二从下载、审计、manifest 到规范化候选的执行清单。
- `TASK4_DATASET_PREP.md`：任务四教育题目与实验讲义结构化解析的数据清洗、审查和 schema 设计。
- `TASK4_ANNOTATION_GUIDE.md`：任务四中文化学试卷与实验讲义的人工标注口径。
- `config/`：RegionOCR 标签映射和数据集规则。
- `scripts/`：数据审计、manifest 构建、COCO 转换和准备度检查脚本。
...
注意：PatCID 全量数据规模很大，不建议直接推送到 GitHub。优先提交脚本、manifest、清洗后的标注、小规模 benchmark 或下载说明。
...
python scripts\build_regionocr_regions_from_coco.py --help
